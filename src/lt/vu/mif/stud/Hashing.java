...
public class Hashing {
    ...
    public static String INPUT_PADDING = "Utmileo,porta3in5quam6sed7hendrerit24ultrices1neque.Nam4vestib.";

    public static String fixLengthOfInput(String inputString) {
        int inputLength = inputString.length();
        if (inputLength == OUTPUT_LENGTH) {
            return inputString;
        } else if (inputLength < OUTPUT_LENGTH) {
            return (inputString + INPUT_PADDING).substring(0, 64); //adds padding
        } else {
            return inputString.substring(inputLength - 64, inputLength); //hashes only last 64 characters
        }
    }
    ...
    public static String hash(String inputString) {
        ...
        BigInteger result = new BigInteger(conversionIteration3).mod(new BigInteger(conversionIteration2)); //do modulo arithmetics
